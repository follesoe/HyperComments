...
namespace HyperComments
{
    ...
    /// </remarks>
    public abstract class IntraTextAdornmentTagTransformer<TDataTag, TAdornment>
        ...
        where TAdornment : UIElement
    {
        ...
        protected override IEnumerable<Tuple<SnapshotSpan, PositionAffinity?, TDataTag>> GetAdornmentData(NormalizedSnapshotSpanCollection spans)
        {
            ...
            foreach (IMappingTagSpan<TDataTag> dataTagSpan in this.dataTagger.GetTags(spans))
            {
                ...
                PositionAffinity? affinity = span.Length > 0 ? null : this.adornmentAffinity;
